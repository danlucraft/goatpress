...
func TestPlayGameWithWinner(t *testing.T) {
  ...
  if state.WhoseMove != 2 { t.Errorf("second move is for player 2") }
  if state.Score1 != 5 { t.Errorf("Score1 is not 5") }
  if state.Score2 != 0 { t.Errorf("Score2 is not 0") }
  ...
  if state.WhoseMove != 1 { t.Errorf("third move is for player 1") }
  if state.Score1 != 5 { t.Errorf("Score1 is not 5") }
  if state.Score2 != 5 { t.Errorf("Score2 is not 5") }
  ...
  state = game.CurrentGameState()
  if len(state.Moves) != 5 { t.Errorf("wrong number of moves", len(state.Moves), 5) }
  expColorString = "11111 22222 11111 22222 11111"
  if state.ColorString != expColorString {
    t.Errorf("moving didn't update colors correctly", state.ColorString, expColorString)
  }
  if state.WhoseMove != 0 { t.Errorf("should be game over") }
  if state.Score1 != 15 { t.Errorf("Score1 is not 15") }
